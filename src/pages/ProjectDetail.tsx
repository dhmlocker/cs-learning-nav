...
export default function ProjectDetail() {
  ...
  // ===== 统一目录计算（taskFlow > featureModules > developmentSteps） =====
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        {project.finalOutcome && (
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
            <div className="text-xs text-blue-400 font-medium mb-1">最终产出</div>
            <p className="text-sm text-blue-800">{project.finalOutcome}</p>
          </div>
        )}
        <div className="space-y-3">
          {project.sourceBasis && (
            <div className="flex items-center gap-2 text-xs text-gray-500">
              <span className="text-gray-400">来源依据：</span>
              <span className="bg-gray-100 px-1.5 py-0.5 rounded">{project.sourceBasis}</span>
            </div>
          )}
          {project.resourceStatus && (
            <div className="flex items-center gap-2 text-xs text-gray-500">
              <span className="text-gray-400">资源状态：</span>
              <span className={`px-1.5 py-0.5 rounded border ${STATUS_COLORS[project.resourceStatus]}`}>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        {project.targetUsers && project.targetUsers.length > 0 && (
          <div>
            <h2 className="text-sm font-semibold text-gray-800 mb-2">适合人群</h2>
        ...
        {project.prerequisites && project.prerequisites.length > 0 && (
          <div>
            <h2 className="text-sm font-semibold text-gray-800 mb-2">前置要求</h2>
            <ul className="space-y-1">
      ...
      {hasDirectory && (
        <section className="mt-8 border-t border-gray-100 pt-6">
          <h2 className="text-base font-semibold text-gray-800 mb-1">{directoryLabel}</h2>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        {item.count > 0 && (
                          <div className="mt-2">
                            <span className="text-xs text-gray-400">{item.count} 个检查项</span>
                          </div>
                        )}
                      </div>
