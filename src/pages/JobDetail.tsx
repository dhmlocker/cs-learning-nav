  ...
  if (!job) {
    return (
      <div className="content-container py-16 text-center">
        <p className="text-slate-400 text-lg mb-4">岗位不存在</p>
        <Link to="/jobs" className="link text-sm">← 返回岗位列表</Link>
      </div>
    )
  }
    ...
    <div className="content-container py-8">
      ...
      {job.requiredAbilities && job.requiredAbilities.length > 0 && (
        <section className="mt-8">
          <h2 className="text-base font-semibold text-slate-800 mb-3">核心能力要求</h2>
      ...
      {hasDirectory && (
        <section className="mt-8 border-t border-slate-100 pt-6">
          <h2 className="text-base font-semibold text-slate-800 mb-1">准备模块</h2>
            ...
            {moduleDirectory!.map((item, i) => (
              <div key={item.id} className="flex gap-3">
                <div className="flex flex-col items-center shrink-0">
                  <div className="w-8 h-8 rounded-full bg-orange-500 text-white text-xs font-semibold flex items-center justify-center shadow-sm">
                    {i + 1}
                  </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm font-semibold text-slate-800 group-hover:text-orange-700 transition-colors">
                          {item.title}
                        </h3>
                        {item.summary && (
                        ...
                        {item.count > 0 && (
                          <div className="mt-2">
                            <span className="text-xs text-slate-400">{item.count} 个技能点</span>
                          </div>
                        )}
                      </div>
                      <span className="text-xs text-orange-600 border border-orange-200 bg-orange-50 px-2 py-1 rounded-md group-hover:bg-orange-100 transition-colors shrink-0 font-medium">
                        查看模块 →
                      </span>
                    </div>
                  </Link>
      ...
      {job.learningPlan && job.learningPlan.length > 0 && (
        <section className="mt-8 border-t border-slate-100 pt-6">
          <h2 className="text-base font-semibold text-slate-800 mb-1">推荐学习路径</h2>
          <p className="text-xs text-slate-400 mb-4">从零开始准备该岗位的阶段性学习计划</p>
          <div className="space-y-0">
                ...
                <div className={`flex-1 ${i < job.learningPlan!.length - 1 ? 'pb-4' : ''}`}>
                  <div className="card p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-semibold text-slate-800">{lp.phase}</span>
                      <span className="text-xs text-slate-400">{lp.duration}</span>
                    </div>
                    <p className="text-xs text-slate-500">{lp.focus}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {job.skills && job.skills.length > 0 && (
        <QuestionList title="核心能力" items={job.skills} colorClass="text-emerald-500" />
      )}

      {job.tasks && job.tasks.length > 0 && (
        <QuestionList title="典型任务" items={job.tasks} colorClass="text-amber-500" className="mt-6" />
      )}

      {job.portfolioAdvice && job.portfolioAdvice.length > 0 && (
        <section className="mt-8 border-t border-slate-100 pt-6">
          <h2 className="text-base font-semibold text-slate-800 mb-3">作品集 / 简历建议</h2>
          <ul className="space-y-2">
      ...
      {job.interviewFocus && job.interviewFocus.length > 0 && (
        <section className="mt-6">
          <h2 className="text-base font-semibold text-slate-800 mb-3">面试考察重点</h2>
      ...
      {job.growthPath && job.growthPath.length > 0 && (
        <section className="mt-8 border-t border-slate-100 pt-6">
          <h2 className="text-base font-semibold text-slate-800 mb-3">职业成长路径</h2>
          <div className="space-y-2">
            {job.growthPath.map((item, i) => (
              <div key={i} className="flex gap-3 items-start card p-3">
                <span className="w-6 h-6 rounded-full bg-brand-600 text-white text-xs font-semibold flex items-center justify-center shrink-0 shadow-sm">
                  {i + 1}
                </span>
                <p className="text-sm text-slate-600 pt-0.5">{item}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {job.commonMistakes && job.commonMistakes.length > 0 && (
        <section className="mt-6">
          <h2 className="text-base font-semibold text-slate-800 mb-3">新手常见误区</h2>
