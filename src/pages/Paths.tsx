...
const STATS: { key: string; label: string; color: string }[] = [
  ...
  { key: 'courses', label: '课程', color: 'bg-emerald-50 text-emerald-600' },
...
export default function Paths() {
  ...
  return (
    <div className="page-container py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900 mb-1">学习路径</h1>
        <p className="text-sm text-slate-500">根据不同目标方向组织的阶段化学习路线</p>
      </div>
          ...
          return (
            ...
            >
              ...
              <p className="text-sm text-slate-500 mb-3">
                {p.description} · 目标岗位：<span className="text-slate-600 font-medium">{p.targetJob}</span>
              </p>

              <div className="flex flex-wrap items-center gap-2 mb-3">
                {STATS.map(({ key, label, color }) => (
                  <span key={key} className={`text-xs font-medium px-2 py-0.5 rounded ${color}`}>
              ...
              <div className="flex items-center gap-1.5 text-xs text-slate-400">
                <span className="text-slate-500 font-medium">学习路线：</span>
                {p.stages.map((s, i) => (
